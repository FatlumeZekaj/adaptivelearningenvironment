...
<form action="<?php echo site_url('teacher/teachercourses/lessons/'.$param3.'/edit'.'/'.$param2); ?>" method="post" enctype="multipart/form-data">
...
function ajax_get_section(course_id) {
    $.ajax({
        url: '<?php echo site_url('teacher/teachercourses/ajax_get_section/');?>' + course_id ,
...
function ajax_get_video_details(video_url) {
    ...
    if(checkURLValidity(video_url)){
        $.ajax({
            url: '<?php echo site_url('teacher/teacherr/ajax_get_video_details');?>',
            type : 'POST',
